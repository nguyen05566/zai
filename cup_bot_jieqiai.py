"""
cup_bot_jieqiai.py — Cờ Úp Bot dùng engine JieqiAI (ElephantEye 揭棋 fork, GPLv3)
https://github.com/arthuryangcs/JieqiAI — build qua build_engine_jieqiai.sh
(đã vá 5 bug nghiêm trọng của bản gốc, xem jieqiai_patches.diff)

Khác biệt vs cup_bot_mistboard.py:
- Giao thức UCCI (ucci/ucciok), KHÔNG phải UCI
- position = FEN ĐẦY ĐỦ mỗi nước (board + side + bag mỗi bên) — không dùng
  moves-list vì macro reveal TRUE_PC_MOVED/UNKNOWN_CPT của engine trả nhầm
  side thay vì piece-code
- go time <budget_ms> (UCCI không có wtime/btime) — TM tự tính min(cap, clock/15)
- Thay searchmoves bằng banmoves (được jieqiai_adapter.bestmove_legal quản lý:
  cấm từng nước lệch luật + retry; fallback nước hợp lệ nếu engine inexplicably
  đơ — KHÔNG BAO GIỜ bỏ lượt)
...
# === ENGINE CONFIG ===
# Engine binary + UCCI wrapper: xem jieqiai_adapter.py (env JIEQIAI_ENGINE)
# Candidates: $JIEQIAI_ENGINE, <bot_dir>/jieqiai, ./jieqiai — adapter tự chmod +x
...
class XiangqiBoardTracker:
    ...
    def get_jieqiai_fen(self):
        """FEN đầy đủ cho engine JieqiAI (UCCI):
        <board> <w|b> <bagR> <bagREaten> <bagB> <bagBEaten>
        - board dựng 100% từ thông tin CÔNG KHAI: start_fen + uci_moves
          (quân úp đi -> lật theo suffix; quân úp bị ăn không lật — model GameVH)
        - bag = 15 loại trừ các quân đã lật công khai mỗi bên
        - 2 space đuôi: chống overread vòng parse bag cuối (đã patch engine)
        """
        revealed = {'r': {}, 'b': {}}
        grid = {}  # pos (row*9+col, row 0 = ĐỎ dưới) -> ký tự FEN
        row = 9
        col = 0
        for ch in self.start_fen:
            if ch == '/':
                row -= 1
                col = 0
            elif ch.isdigit():
        ...
        for uci in self.uci_moves:
            if not UCI_MOVE_WITH_SUFFIX_RE.match(uci):
                continue
            mv, suffix = uci[:4], (uci[4:] or None)
            try:
                src_pos, dst_pos = self.engine_move_to_pos(mv)
            except Exception:
                break
            piece = grid.get(src_pos, '.')
            if suffix:
                # màu quân LẤT = màu quân ĐI (case của ký tự trên ô nguồn),
                # KHÔNG phụ thuộc case suffix (server có thể gửi thường/hoa)
                ch = suffix.upper() if piece.isupper() else suffix.lower()
                t = ch.lower()
                if t in ('a', 'b', 'n', 'r', 'c', 'p'):
                    key = 'r' if piece.isupper() else 'b'
                    revealed[key][t] = revealed[key].get(t, 0) + 1
                grid[dst_pos] = ch
                if src_pos != dst_pos:
                    grid.pop(src_pos, None)
            else:
                if src_pos == dst_pos or piece == '.':
                    continue
                grid[dst_pos] = piece
                grid.pop(src_pos, None)
        rows = []
        for r in range(9, -1, -1):
            ...
            if empty:
                s += str(empty)
            rows.append(s)
        for ch in self.start_fen:  # quân lật sẵn trong bố cục gốc
            if ch.isalpha() and ch not in ('X', 'x'):
                key = 'r' if ch.isupper() else 'b'
                t = ch.lower()
                if t in ('a', 'b', 'n', 'r', 'c', 'p'):
                    revealed[key][t] = revealed[key].get(t, 0) + 1
        # (các lần lật từ moves đã đếm ở vòng lặp trên, màu theo quân đi)
        bags = []
        for key in ('r', 'b'):
            full = {'a': 2, 'b': 2, 'n': 2, 'r': 2, 'c': 2, 'p': 5}
            for t, n in revealed[key].items():
                full[t] = max(0, full[t] - n)
            bags.append("".join(t * full[t] for t in ('r', 'n', 'b', 'a', 'c', 'p')))
        side = self.side_to_move if self.side_to_move in ('w', 'b') else 'w'
...
class JieqiAIBotEngine:
    """UCCI wrapper cho engine JieqiAI — interface giống wrapper pikafish
    (get_best_move trả 'bestmove xxxx') để phần còn lại của bot giữ nguyên."""
    ...
    def __init__(self):
        ...
        self._last_score = "?"
        try:
            from jieqiai_adapter import JieqiAIEngine
            self.adapter = JieqiAIEngine()
            self.engine = True
            print(f"[ENGINE] 🎯 jieqiai = {self.adapter.binary_path}")
            print("[ENGINE] ✅ jieqiai (UCCI) ready")
        except Exception as e:
            print(f"[ENGINE] ❌ jieqiai không khởi động được: {e}")

    def alive(self):
        return self.adapter is not None and self.adapter.alive()
    ...
    def restart(self):
        ...
        try:
            from jieqiai_adapter import JieqiAIEngine
            self.adapter = JieqiAIEngine()
            print("[ENGINE] 🔄 restarted jieqiai")
            return self.alive()
        except Exception as e:
            print(f"[ENGINE] ❌ restart fail: {e}")
            self.adapter = None
            return False

    def get_best_move(self, jieqiai_fen, moves=None, movetime_ms=2000,
                      wtime_ms=None, btime_ms=None, cap_seconds=None,
                      searchmoves=None):
        """Trả 'bestmove xxxx' hoặc None. TM: budget = min(cap, clock/15)."""
        if not self.alive():
            if not self.restart():
                return None
        budget_ms = int(float(cap_seconds) * 1000) if cap_seconds else movetime_ms
        if wtime_ms is not None:
            budget_ms = min(budget_ms, max(150, int(wtime_ms / 15)))
        budget_ms = max(120, min(budget_ms, 20000))
        try:
            if searchmoves:
                mv, banned = self.adapter.bestmove_legal(jieqiai_fen, budget_ms,
                                                         list(searchmoves))
            else:
                # không có danh sách hợp lệ (rules off) — tin engine thô
                mv = self.adapter.bestmove(jieqiai_fen, budget_ms)
                banned = []
        except Exception as e:
            print(f"[ENGINE] ❌ lỗi engine: {e}")
            return None
        if mv is None:
            return None
        if banned:
            print(f"[ENGINE] ⚠️ đề xuất sai luật {banned} -> đã cấm, chọn {mv}",
                  flush=True)
        info = getattr(self.adapter, "_last_info", "")
        m = re.search(r'depth (\d+)', info)
        if m:
            self._last_depth = m.group(1)
        sm = re.search(r'score (-?\d+)', info)
        if sm:
            self._last_score = sm.group(1)
...
class JieqiCupBot:
    ...
    def _do_auto_move(self):
        ...
        jfen = self.board.get_jieqiai_fen()
        print(f"[ENGINE-IN] FEN: {fen[:80]}...", flush=True)
        print(f"[ENGINE-IN] JieqiAI-FEN: {jfen[:110]}", flush=True)
        print(f"[ENGINE-IN] moves({len(moves)}) | clock me={self._my_clock_s:.1f}s "
              ...
              flush=True)
        raw = self.engine.get_best_move(jfen, moves,
