...
void parsefen(string f){pos=State{};pos.b.fill('.');stringstream ss(f);string board,side;ss>>board>>pos.bag>>side;int r=0,c=0;for(char ch:board){if(ch=='/'){r++;c=0;continue;}if(isdigit((unsigned char)ch))c+=ch-'0';else if(r<10&&c<9)pos.b[r*9+c++]=ch;}pos.red=side!="b";}
